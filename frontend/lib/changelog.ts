...
export const changelog: ChangeEntry[] = [
  {
    date: '2026-06-12',
    changes: [
      { type: 'new', text: '分析プラン「ディープ分析」を追加（上位AIモデルによる深掘り分析・2クレジット）。レポート画面に分析プランのバッジを表示' },
      { type: 'improve', text: 'トップページに2つの分析プランの比較セクションを追加（同じセッションを両プランで分析した実例つき）。分析画面にプランの使い分けの目安を表示' },
      { type: 'improve', text: '通常分析のAIモデルを最新版に更新（分析精度が向上）' },
      { type: 'improve', text: 'ICF資格判定の表示文言を見直し（「参考水準を満たしています／あと少し／要練習」）' },
